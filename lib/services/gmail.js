...
const { createExternalServiceError, logErrorToMonitoring } = require('../errors/error-handler');
...
class GmailService {
  ...
  getAuthUrl(options = {}) {
    try {
      ...
      logErrorToMonitoring(error, 'Failed to generate Gmail OAuth URL');
  ...
  async getTokens(code) {
    try {
      ...
      logErrorToMonitoring(error, 'Gmail OAuth token exchange failed', {
        hasCode: !!code,
  ...
  async getEmails(options = {}) {
    try {
      ...
      logErrorToMonitoring(error, 'Failed to fetch emails from Gmail');
      throw createExternalServiceError(
        'gmail',
  ...
  async getEmailDetails(messageId) {
    try {
      if (!this.isAuthenticated) {
        throw createExternalServiceError(
          'gmail',
          'Gmail service is not authenticated. Please call getTokens() first.'
        );
      }
      ...
      if (!messageId) {
        throw createExternalServiceError(
          'gmail',
          'Message ID is required to fetch email details'
        );
      }
      ...
      logErrorToMonitoring(error, `Failed to fetch email details for ${messageId}`, {
        messageId
      });
  ...
  async getEmail(messageId) {
    try {
      ...
      logErrorToMonitoring(error, `Failed to fetch email ${messageId} from Gmail`);
      throw createExternalServiceError(
        'gmail',
        `Failed to fetch email with ID ${messageId}`,
        error
      );
    }
  }
  ...
  async sendEmail(options) {
    try {
      ...
      if (!to || !subject || !body) {
        throw createExternalServiceError(
          'gmail',
          'Missing required email fields: to, subject, and body are required'
        );
      }
      ...
      logErrorToMonitoring(error, 'Failed to send email via Gmail');
      throw createExternalServiceError(
        'gmail',
  ...
  async markAsRead(messageId, read = true) {
    try {
      ...
      logErrorToMonitoring(error, `Failed to mark email ${messageId} as ${read ? 'read' : 'unread'}`);
      throw createExternalServiceError(
        'gmail',
  ...
  async modifyLabels(messageId, options) {
    try {
      ...
      logErrorToMonitoring(error, `Failed to modify labels for email ${messageId}`);
      throw createExternalServiceError(
        'gmail',
        'Failed to modify email labels',
        error
      );
    }
  }
  ...
  async getProfile() {
    try {
      ...
      logErrorToMonitoring(error, 'Failed to fetch Gmail profile');
      throw createExternalServiceError(
        'gmail',
  ...
  async searchEmails(keywords, options = {}) {
    try {
      ...
      if (!keywords) {
        throw createExternalServiceError(
          'gmail',
          'Keywords parameter is required for email search'
        );
      }
      ...
      logErrorToMonitoring(error, `Failed to search emails with keywords: ${keywordsStr}`, {
